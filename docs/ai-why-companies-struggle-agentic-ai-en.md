...
And there's one link in this chain that's the most insidious and least discussed: **when nobody set up a CI check that should have been there, whose fault is that?** The earlier links — writing, reviewing, merging — are all "someone did something": there's an action, a timestamp, a trail to follow. But "a missing check" is "someone *didn't* do something": no action, no log, not a single record that pops up to say "there should have been a gate here." When something breaks, you may not even notice what was missing, because **an absence doesn't throw an error**. This has a traditional name — the *sin of omission* — and the sin of omission is always harder to see and harder to trace than the sin of commission. Underneath it is, again, moral hazard: setting up a gate has a cost (it takes time, and it blocks your own PRs too), while whoever skips it saves effort now and externalizes the cost onto some future reviewer who inherits the mess. So "not setting up the check" is itself an incentive structure that rewards cutting corners.

How do you attribute it? Layering makes it clear again: whether the check *should exist* (should there be a gate at all) is a platform/infrastructure-layer responsibility — typically the tech lead or platform team, not the individual PR author; whether the check is *configured correctly* is the capability-boundary responsibility of whoever set it up; and a check being *bypassed or disabled* (someone skips CI, force-merges) is actually the easiest to trace, because that's an action with a trail — provided you have auditing. The real fix lives inside that "capability boundary list" from the solutions: it can't just govern what the agent may do, it must also govern **which checks must exist and cannot be silently skipped.** Because a gate that's mandatory but can be quietly switched off is no gate at all. Make required checks org-level enforced, with any bypass leaving a named trail — and "there's no check here" turns from an ownerless vacuum back into a decision with someone's name on it.
...
This isn't just theory. Industry cases already describe companies running this kind of automation loop to process thousands of PRs a week — yet **every one is still reviewed by a human**. People haven't left the process; their seat changed, from writing code to reviewing it. More telling still, some of those cases made a counterintuitive choice: rather than the most powerful model, they paired a smaller model with strict automated gates. The lesson matters especially for leadership — **for loop-based work, one solid verification gate matters more than raw model power.** In other words, what decides whether this scales isn't how smart the AI is, but how well you've designed your accountability gates.
...
The moment you say "set up a gate," anyone who's spent time in a company knows there's a long-standing fault line here: the security team wants gates and control, RD wants speed and autonomy. Agents didn't create this fault line — they amplify one that **already existed**, pushing it to a breaking point.

This tension is well-documented empirically; it's not your imagination. A systematic review of DevSecOps found that of all the "people" problems, the one flagged by the most papers is the conflict between development and security teams — developers feel the security team judges and nitpicks their work, and resent being stripped of their development autonomy. Another empirical study captured a practitioner quote vivid enough to memorize verbatim: developers see security as a "blocker," while the security team feels developers are trying to shove them aside and move faster than they can keep things secure — the result being that both sides adopt an adversarial rather than a collaborative stance.

And the key point: this **isn't about anyone being bad or a failure to communicate — the objective functions genuinely differ.** RD's goal is to ship to production as fast as possible; security's goal is a thorough assessment of the system — and fast delivery without full checks is in direct conflict with the latter. Both sides are doing the "right" thing; they just define "right" differently. That's why it's a structural conflict that no amount of "let's all collaborate" will talk away.

Now drop an agent onto this fault line and you notice something: every remedy I'm about to prescribe — enforced CI gates, capability boundary lists, human merge review, audit trails — **every one is something security wants, and every one is something RD instinctively resists (slow, in the way, being watched).** My solutions land squarely on top of the organizational crack.

But here's the twist, and it's the one thought I most want to leave with leadership: **agents have a chance to turn these old enemies into allies.** Because the fault line has moved. Security used to gate "human-written code that RD was confident in," so RD felt nitpicked. But now, both sides face "agent-written code that not even RD is fully confident in." It's the first time RD and security share the *same* enemy — the AI output no one can fully trust. Security's gate used to feel like policing RD; in the agent era, that gate protects RD — it protects the engineer who'd otherwise carry the consequences of the AI's output alone. Whoever can make the team see that shift can turn the deepest organizational resistance into a driving force.
...
**First, make the agent observable before you talk about autonomy.** The concrete bar: every action the agent takes — which tool it called, which file it changed, on what basis it decided — must map to a traceable log line, the kind you'd read when debugging a normal bug. Until you can do that, don't talk about autonomy. A human review checkpoint at this stage isn't the enemy of autonomy — it's the entry ticket that gives the agent a chance to be accepted. Understand it first, then loosen the reins.
